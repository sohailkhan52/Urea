...
const localRuntimeDir = join(import.meta.dirname, 'build', 'php-runtime');
...
if (platform.phpBinary) {
    try {
        if (isWindows && fs.existsSync(join(localRuntimeDir, platform.phpBinary))) {
            console.log('Copying local PHP runtime from ' + localRuntimeDir + ' to ' + binaryDestDir);
            removeSync(binaryDestDir);
            fs_extra.copySync(localRuntimeDir, binaryDestDir);
            process.exit(0);
        }
