...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-1">Payment History</h1>
    ...
    <div class="card mb-4 no-print">
        ...
        <div class="collapse show" id="filterPanel">
            <div class="card-body">
                <form method="GET" action="{{ route('admin.reports.customer.payment-history', $customer) }}">
                    <div class="row g-3">
                        <div class="col-md-2">
            ...
            <div class="card h-100">
                ...
                <div class="card-body p-0">
                    ...
                    @php
                        ...
                        $methodColors = ['cash'=>'success','bank_transfer'=>'info','easypaisa'=>'warning',
                                         'jazz_cash'=>'warning','cheque'=>'secondary','other'=>'light'];
                        $color = $methodColors[$method->payment_method] ?? 'secondary';
                    @endphp
                    <div class="px-3 py-2 {{ !$loop->last ? 'border-bottom' : '' }}">
                        <div class="d-flex justify-content-between">
                            <div>
                                <span class="badge bg-{{ $color }} {{ in_array($color, ['warning','light']) ? 'text-dark' : '' }}">
                                    {{ \App\Models\Payment::$methods[$method->payment_method] ?? ucfirst($method->payment_method) }}
                                </span>
                        ...
                        <table class="table table-hover table-sm mb-0 align-middle">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if($payment->sale)
                                            <a href="{{ route('admin.sales.show', $payment->sale_id) }}"
                                    ...
                                    <td>
                                        @php
                                            $mc = ['cash'=>'success','bank_transfer'=>'info','easypaisa'=>'warning',
                                                   'jazz_cash'=>'warning','cheque'=>'secondary','other'=>'light'];
                                            $mc2 = $mc[$payment->payment_method] ?? 'secondary';
                                        @endphp
                                        <span class="badge bg-{{ $mc2 }} {{ in_array($mc2,['warning','light'])?'text-dark':'' }}">
                                    ...
                                    <td>
                                        @if($payment->receiver)
                                            <small>{{ $payment->receiver->name }}</small>
                                        @else
                                            <span class="text-muted">—</span>
                                        @endif
                                    </td>
                                    <td>
                                        <small class="text-muted">
                                            {{ $payment->notes ? \Illuminate\Support\Str::limit($payment->notes, 35) : '—' }}
                                        </small>
                                    </td>
                                </tr>
                                @endforeach
