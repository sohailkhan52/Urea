...
<form id="bulkDeleteForm" action="{{ route('admin.reports.sales.bulk-delete') }}" method="POST" style="display: none;">
    @csrf
    @method('DELETE')
    <!-- sale_ids will be added dynamically by JavaScript -->
</form>
...
<script>
    ...
    function executeBulkDelete() {
        ...
        if (!confirm(`Delete ${saleIds.length} sale(s)? This action cannot be undone.`)) {
            return;
        }
        ...
        fetch('{{ route("admin.reports.sales.bulk-delete") }}', {
            method: 'DELETE',
            body: formData,
            headers: {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                setTimeout(() => {
                    window.location.href = '{{ route("admin.reports.sales.index") }}';
                }, 1000);
            } else {
